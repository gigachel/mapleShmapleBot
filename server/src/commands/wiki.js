...
import { Menu } from "@grammyjs/menu"; // MenuRange
...
export async function searchWikiWithVariants(ctx, search) {
  ...
  const articlesList = await (await fetch("https://ru.wikipedia.org/w/api.php?" + urlParams)).json();
...
export async function getWikiExtract(title) {
  ...
  const article = await (await fetch("https://ru.wikipedia.org/w/api.php?" + urlParams)).json();
